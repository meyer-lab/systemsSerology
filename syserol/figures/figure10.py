...
def makeFigure():
    ...
    for i, j in zip(index, place):
        ...
        a = sns.scatterplot(x=f"Component {i+1} Measurement", y=f"Component {i+2} Measurement", 
                            hue="Groups", data=df, palette="Set1", legend="brief", ax=ax[j])
        ...
        df = pd.DataFrame(data)
        b = sns.scatterplot(x=f"Component {i+1} Measurement", y=f"Component {i+2} Measurement", 
                            hue="Groups", data=df, palette="Set2", legend="brief", ax=ax[j+1])
        ...
        df = pd.DataFrame(data)
        c = sns.scatterplot(x=f"Component {i+1} Measurement", y=f"Component {i+2} Measurement", 
                            hue="Groups", data=df, palette="Set3", legend="brief", ax=ax[j+2])
        c.legend(loc='center left', bbox_to_anchor=(1, 0.5), ncol=1)

        # Glycans/Functions
        values1 = glyc[:, i]
        values2 = glyc[:, i+1]
        data = {f"Component {i+1} Measurement":values1, f"Component {i+2} Measurement":values2,
                "Groups":np.concatenate((np.array(glycaninf["glycan"]), functions))}
        df = pd.DataFrame(data)
        d = sns.scatterplot(x=f"Component {i+1} Measurement", y=f"Component {i+2} Measurement", 
                            hue="Groups", data=df, palette="muted", legend="brief", ax=ax[j+3])
