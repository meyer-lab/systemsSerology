...
def perform_CMTF(tensorIn, matrixIn, r):
    ...
    kruskal = parafac(tensor, r, mask=mask, orthogonalise=True, normalize_factors=False, tol=1e-10, n_iter_max=1000000, linesearch=True, verbose=True)
    tensor = tensor*mask + tl.kruskal_to_tensor(kruskal, mask=1 - mask)
    assert np.all(np.isfinite(tensor))

    # Now run CMTF
    tensorFac, matrixFac = coupled_matrix_tensor_3d_factorization(tensor, matrix, r, mask_3d=mask, verbose=True, mask_matrix=mask_matrix, init=kruskal)
