...
Unlike in earlier work [@DOI:10.15252/msb.20177881], we employed a nested, 30-fold cross-validation strategy, wherein the regularization was determined separately within each cross-validation fold. Hyperparameter selection outside of each cross-validation fold is known to consistently lead to model over-fitting [@overfit]. Cross-validation was stratified by class for classification. All subjects were randomly shuffled before classification to prevent the influence of subject ordering in the dataset. For the factorization-based predictions, the better performing of either a linear or Gaussian process model was used.
