...
Unlike in earlier work [@DOI:10.15252/msb.20177881], we employed a nested, 20-fold cross-validation strategy, wherein the regularization extent was determined separately within each cross-validation fold. Hyperparameter selection outside of each cross-validation fold is known to consistently lead to model over-fitting [@id:overfit]. When evaluating predictions using the factorization, both regularized linear and Gaussian process models were built, and the better performing model selected.
